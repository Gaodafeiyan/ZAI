...
import PancakePairABI from '../contracts/PancakePair.json';
...
export default function Swap() {
  ...
  const [pairReserves, setPairReserves] = useState({ zusd: '0', zai: '0' });
  ...
  const checkPairLiquidity = async () => {
    try {
      const provider = new ethers.BrowserProvider(window.ethereum);
      const pairContract = new ethers.Contract(
        CONTRACTS.ZAI_ZUSD_PAIR,
        PancakePairABI.abi,
        provider
      );
      ...
      // 确定哪个是 ZUSD，哪个是 ZAI
      const isToken0ZUSD = token0.toLowerCase() === CONTRACTS.ZUSD.toLowerCase();
      const zusdReserve = isToken0ZUSD ? reserves[0] : reserves[1];
      const zaiReserve = isToken0ZUSD ? reserves[1] : reserves[0];

      setPairReserves({
        zusd: ethers.formatUnits(zusdReserve, 18),
        zai: ethers.formatUnits(zaiReserve, 18)
      });
      ...
      if (zusdReserve === 0n || zaiReserve === 0n) {
        toast.warning('⚠️ 流动性池尚未初始化，需要添加流动性');
      }
    } catch (error) {
  ...
  const handleDEXSwap = async () => {
    try {
      ...
      // 先检查流动性池储备
      if (parseFloat(pairReserves.zusd) === 0 || parseFloat(pairReserves.zai) === 0) {
        toast.error('❌ 流动性池为空，需要先添加流动性！');
        toast.info('💡 请联系管理员添加流动性或使用 OTC 模式兑换');
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        // 提供更详细的错误信息
        if (error.message && (error.message.includes('INSUFFICIENT_LIQUIDITY') || error.message.includes('K'))) {
          toast.error('❌ 流动性不足！当前池子储备:');
          toast.info(`ZUSD: ${parseFloat(pairReserves.zusd).toFixed(2)} | ZAI: ${parseFloat(pairReserves.zai).toFixed(2)}`);
          toast.warning('请减少兑换金额或等待流动性增加');
        } else {
          toast.error('无法获取兑换价格，请稍后重试');
        }
        setLoading(false);
        return;
      }
  ...
  return (
    <Container maxWidth="sm" sx={{ py: 4 }}>
      <MotionCard
        ...
        sx={{
          background: 'linear-gradient(135deg, #fdb714 0%, #f6841f 100%)', // PancakeSwap 橙色主题
      ...
      >
        <CardContent sx={{ p: 4 }}>
          ...
          {/* 显示流动性池信息 */}
          {((fromToken === 'ZUSD' && toToken === 'ZAI') || (fromToken === 'ZAI' && toToken === 'ZUSD')) && (
            <Alert severity="info" sx={{ mb: 2 }}>
              流动性池储备 - ZUSD: {parseFloat(pairReserves.zusd).toFixed(2)} | ZAI: {parseFloat(pairReserves.zai).toFixed(2)}
            </Alert>
          )}

          <Collapse in={showSettings}>
            <Paper sx={{ p: 2, mb: 3, backgroundColor: 'rgba(255,255,255,0.15)' }}>
          ...
          <Paper sx={{ p: 3, mb: 2, backgroundColor: 'rgba(255,255,255,0.15)' }}>
          ...
          <Paper sx={{ p: 3, mb: 3, backgroundColor: 'rgba(255,255,255,0.15)' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
          ...
          >
            {loading && <CircularProgress size={24} sx={{ mr: 1, color: '#f6841f' }} />}
            {getButtonText()}
          </Button>
